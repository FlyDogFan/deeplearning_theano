...
def test_RNN(learning_rate=0.01, n_epochs=1000,
                           dataset='changhen.txt'):
    
    print "... loading data"
    
    train_set_x_value, train_set_y_value, vocabulary = load_data(dataset)
    
    n_train = len(train_set_x_value)
    ...
    for echo in range(n_epochs):
        ...
        for j in range(n_train):
            loss += train_model(train_set_x_value[j], train_set_y_value[j])
...
def load_data(dataset):
    ...
    return (train_set_x_value, train_set_y_value, vocabulary)
